...
if ( ! class_exists( 'Wp_License_Manager_Client' ) ) {
    ...
    class Wp_License_Manager_Client {
        ...
        public function add_license_settings_page() {
          $title = sprintf( __( '%s', $this->text_domain ), $this->product_name );
        ...
        public function wub_plugin_settings() {
          ?>
          ...
          <div class="wrap">
            <h1><?php echo $this->admin_page_title; ?></h1>
            <div class="update-nag wub_ajax_update" style="border-left-color: #46b450;margin-bottom: 30px; margin-top: 0;">Select a taxonomy to represent the SEO url of each post type</div>
            <table>
              <?php
              foreach (get_post_types(['public'=>true, '_builtin'=>false], 'names') as $post_type) {
                $taxonomy_objects = get_object_taxonomies($post_type, 'objects');
                include(WUB_PLUGIN_PATH . 'templates/wub-new-option.php');
              }
              ?>
              <tr>
                <th scope="row" style="text-align: left; width: 20%;" >Flush on init</th>
                <td>
                  <input type="checkbox" class="flush_on_post_save">
                  <td>
                    <p class="desciption"><i>
                     Your permalinks will get flushed every time you load your page,
                     This is resource heavy so saving so your site speed may slow down while this is active (not recommended for production)
                   </i>
                 </p>
               </td>
             </td>
           ...
           <tr>
            <td style="vertical-align: bottom;">
              <input type="button" name="flush_permalinks" value="Flush Permalinks" class="button button-default flush_permalinks">
            </td>
          </tr>
      ...
      <script type="text/javascript" >
        jQuery(document).ready(function($) {
          ...
          var data = {
            'action': 'wub_get_flush_checked',
          };
          $.post(ajaxurl, data, function(response) {
            if(response.length) {
             $('.flush_on_post_save').prop("checked", true);
           }
         });


          var data = {
            'action': 'wub_get_all_post_type',
          };
          $.post(ajaxurl, data, function(response) {
            ...
            for (var i = data.length - 1; i >= 0; i--) {
              ...
              $('input[data-post_type="'+ data[i].name + '"][value="' + data[i].value + '"]').prop("checked", true);
            }
          });

          $('.wub_post_type_option').change(function(event) {
            event.preventDefault();

            var post_type = $(this).data('post_type');
            var wub_post_type_value = $('input[name="wub_post_type_option_' + post_type + '"]:checked').val();

            var data = {
              'action': 'wub_post_type',
              'wub_post_type': post_type,
              'wub_post_type_value': wub_post_type_value
            };
            $.post(ajaxurl, data, function(response) {
              $('.wub_ajax_update').html(response);
            });
          });

          $('.flush_on_post_save').change(function(event) {
            event.preventDefault();

            var checked = $(this).attr('checked');

            var data = {
              'action': 'wub_flush_on_post_save',
              'wub_checked': checked
            };
            $.post(ajaxurl, data, function(response) {
              $('.wub_ajax_update').html(response);
          ...
          $(".flush_permalinks" ).on("click", function(event) {
           event.preventDefault();
           var data = {
            'action': 'wub_flush_permalinks_ajax',
          };
